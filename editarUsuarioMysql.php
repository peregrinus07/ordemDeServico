...
";
   ...
   //$sql = "DELETE FROM cliente WHERE id_cliente = '$id'";
	...
	$sql = "UPDATE tabela_usuario SET nome_usuario='$nome', cpf_usuario='$cpf', e_mail_usuario='$email', telefone_usuario='$telefone' WHERE id_usuario='$id'";

   	// $sql = "SELECT nome_cliente FROM cliente";
    $resultado = mysqli_query($conn,$sql) or die("Erro ao retornar dados");

    $sql="select count(id_endereco_usuario) AS total from tabela_endereco_usuario where fk_id_usuario='$id'";
	...
	print_r("<br>numero endereco: ".$numero);
	   print_r("<br>");
    	...
    	$sql ="UPDATE  tabela_endereco_usuario
SET fk_id_usuario='$id', fk_id_rua='$idRua',numero_endereco_usuario='$numero', cep_endereco_usuario='$cep'
...
";    	
    ...
    if($total['total']==0){
       ...
       print_r("total: ".$total['total']);
      print_r("<br>usuario não cadastraro endereço");

      print_r("<br>id rua: ".$idRua);

      $sql="insert into  tabela_endereco_usuario (fk_id_usuario,fk_id_rua,numero_endereco_usuario) values ('$id','$idRua','$numero')";
	...
	echo "<span class='news2'><br>Dados atualizados com sucesso.</span>";
 	echo "<br> <a href='listarUsuarios.php'>Voltar</a>";
/*
